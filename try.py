    ...
    def __init__(self, root):
        ...
        for i, label_text in enumerate(labels):
            ...
            if label_text == "Amount":
                entry = tk.Entry(self.root, width=50)  # Increase width for Amount entry box
                entry.grid(row=i, column=1, padx=20, pady=10, sticky="w")
            elif label_text == "Currency" or label_text == "Category" or label_text == "Payment Method":
                values = ["USD", "GBP", "EGP", "SAR", "AED", "gas"]
                entry = ttk.Combobox(self.root, values=values, state='readonly', width=55)  # Increase width for Comboboxes
                entry.set(values[0])
                entry.grid(row=i, column=1, padx=20, pady=10, sticky="w")
            elif label_text == "Date":
                entry = DateEntry(self.root, width=40)  # Increase width for DateEntry
                entry.grid(row=i, column=1, padx=20, pady=10, sticky="w")
